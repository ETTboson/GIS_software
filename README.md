...
GeoAI 智能空间分析平台是一个基于 Qt、QGIS、GDAL 与本地 AI 编排能力构建的桌面 GIS 应用原型。项目面向空间数据查看、分析与智能交互场景，提供地图画布、图层加载、统一数据资产管理、统计分析、栅格邻域分析、矢量缓冲区分析以及 AI 辅助调用分析工具的能力。

项目当前重点不是做传统 GIS 的完整替代品，而是探索“桌面 GIS + 数据资产工作区 + AI 工具调用”的集成方式：用户可以通过界面选择数据、调整分析参数，也可以通过 AI 对话表达分析意图，由系统完成参数补齐、工具分发和结果回传。
...
本项目没有把第三方库随仓库一起分发。首次配置 CMake 时需要在本机准备并填写以下依赖根目录：
...
这是一项项目级修复，不需要开启 Windows 控制面板中的“Beta 版：使用 Unicode UTF-8 提供全球语言支持”，也不会影响 CAD 等依赖旧代码页的本机软件。
