...
// ════════════════════════════════════════════════════════
//  ConversationContext
//  职责：管理裸对话历史、system prompt 注入与 ET.md 记忆读写。
//        负责把温数据记忆按需注入消息流，并在历史过长时执行压缩。
//  位于 core/ai/ 层。
// ════════════════════════════════════════════════════════
...
public:
    /*
     * @brief 构造函数
     * @param_1 _strWorkDir: 工作目录，用于扫描 ET.md 与 memory/*.md
...
private:
    ...
    MemoryFileManager    mMemoryFileManager;     // ET 风格记忆文件管理器
