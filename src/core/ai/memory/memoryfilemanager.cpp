// src/core/ai/memory/memoryfilemanager.cpp
...
{
    return QDir(mstrWorkDir).filePath("ET.md");
}
...
QStringList MemoryFileManager::discoverMemoryFiles() const
{
    ...
    const QString _strPrimaryPath = _workDir.filePath("ET.md");
    ...
    for (int _nLevelIdx = 0; _nLevelIdx < 3; ++_nLevelIdx) {
        ...
        const QString _strParentEt = _parentDir.filePath("ET.md");
        if (QFile::exists(_strParentEt) && !_vResult.contains(_strParentEt)) {
            _vResult << _strParentEt;
        }
    }
